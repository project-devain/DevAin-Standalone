...
class ModalGPTCommand(
    ...
    override suspend fun onCommand(event: SlashCommandInteractionEvent) {
        val requestList = mutableListOf<OpenAIGPTMessage>()
        if (event.getOption("base-prompt") != null) {
            requestList.add(
                OpenAIGPTMessage(
        ...
        event.listenModal(createModal("GPT-WhatEver") {
            it.addActionRow(TextInput.create("prompt", "Prompt", TextInputStyle.PARAGRAPH).build())
        }) {
